...
## 從 WSL 觀察與操作客戶端：截圖可行，輸入不行（成因未定）

`[TEST]` 2026-09-16。

| 能力 | 結果 |
|---|---|
| **截圖** | ✅ **可用**。D3D9 視窗模式用 `CopyFromScreen` 截得乾淨，先前擔心的全黑沒有發生。全程自行觀察了啟動、標題、登入畫面 |
| 滑鼠點擊 | ⬜ 疑似可用。點擊後輸入欄位出現游標 |
| 鍵盤輸入 | ❌ **文字不進入遊戲** |

### 鍵盤問題的排查過程（對照組已建立）

1. `SendKeys`（視窗訊息）→ 無效
2. `keybd_event`（真實掃描碼）→ 遊戲無效，**但記事本收到了**，內容為 `_吃吃ㄉ`——按鍵確實送達，被**注音 IME** 攔去組字
3. 送鍵前以 `WM_INPUTLANGCHANGEREQUEST` 切成 en-US → **記事本正常顯示 `ENGLISH-TEST`**，遊戲仍無反應
4. 剪貼簿 + `Ctrl+V` → 遊戲無效

**因此：注入方法本身正確**（記事本全程接收），**遊戲特有地收不到**。

⬜ **成因未定。** 兩個未排除的可能：

- 客戶端執行的 **XIGNCODE3**（`data/System/xigncode.log` 實測執行中、持續寫入）阻擋注入式輸入
- 從背景行程呼叫 `SetForegroundWindow` 對遊戲視窗失敗，按鍵其實送到了別的視窗（記事本剛啟動、具備前景權限，遊戲沒有）

> **先前曾將此逕自判定為「XIGNCODE 阻擋」並寫入台帳，那是證據不足時的結論，已更正。** 對照組是後來才補的。

### 實務結論

**不再投入。** 操作者負責點擊與打字，我負責截圖觀察、讀封包紀錄、讀客戶端 log、改伺服器。

最大的問題來源已經消除——**我不再需要靠轉述判斷畫面**，而那正是今天數次錯誤判斷的根源。

> 本專案界線不變：**不注入行程、不附加除錯器**。截圖是被動觀察；若日後確認為防作弊阻擋，亦不繞過。
