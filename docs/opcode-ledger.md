...
## ❌ 無法從外部驅動客戶端輸入（XIGNCODE3）

`[TEST]` 2026-09-16。嘗試從 WSL 自動操作客戶端，結論：

| 能力 | 結果 |
|---|---|
| **截圖** | ✅ 可用。D3D9 視窗模式用 `CopyFromScreen` 截得乾淨，先前擔心的全黑沒有發生 |
| 鍵盤輸入 | ❌ **無效**。`SendKeys`（視窗訊息）與 `keybd_event`（真實掃描碼）皆試過，文字不進入輸入欄位 |
| 滑鼠點擊 | ⬜ 不確定。一次點擊後輸入欄位出現游標，但無法排除巧合 |

成因：客戶端執行 **XIGNCODE3**（`data/System/xigncode.log`，實測執行中、持續寫入、320 KB）。阻擋注入式輸入是該防作弊的核心功能。

> **不要再嘗試繞過。** 這不是技術挑戰，是該軟體的設計目的；而且本專案的界線是**不注入行程、不附加除錯器**（客戶端另有 y0da CRC、Themida、anti-attach）。截圖是被動觀察，不越線；規避防作弊的輸入偵測是另一回事。

**實際可行的分工**：操作者負責點擊與打字，我負責截圖觀察、讀封包紀錄、讀客戶端 log、改伺服器。這已經消除了最大的問題來源——**我不再需要靠轉述判斷畫面**。
