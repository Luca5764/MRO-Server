...
**在焦點、輸入法、方法皆正確的前提下，客戶端仍然收不到鍵盤輸入。** 客戶端執行 **XIGNCODE3**（`data/System/xigncode.log` 實測執行中、持續寫入），過濾帶注入標記的輸入事件正是其標準行為。

> **不繞過。** 本專案界線：不注入行程、不附加除錯器、不規避防作弊。截圖與滑鼠點擊是被動／一般的桌面操作，不越線。

### 實務分工

操作者負責打字（登入、聊天）；我負責截圖觀察、點擊、讀封包紀錄、讀客戶端 log、改伺服器。
