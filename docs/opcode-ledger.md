...
## ❌ `Game_Info_SN` 不能在房間場景送出（白畫面）

`[TEST]` 2026-09-16。為解決「送太晚」而改在房間狀態區塊送出 `0x00222111`，結果**進房後畫面全白、客戶端停止回應**。

`ZDispatchWaiting::Game_Info_SN` 的流程是先 `Game_Data_Clear()` 再 `Game_Info_Set()`。在房間場景仍在運作時清空遊戲資料，推測連帶把房間畫面也清掉了。

**已還原**：`GAME_INFO_SN_WITH_ROOM_STATE = 'disabled'`。

於是此封包**兩端都不對**：在房間送會清掉畫面，在開始請求之後送則客戶端已經組完 URL。這強烈暗示 **`ZPage_Room` 的地圖來源根本不是這個封包**，而是腳本自己的 `m_MapInfoList`——也就是上一條在處理的東西。
