...
## ▶️ 明天（2026-09-22）要做的事：照順序跑，不要中途問 PM

**時段：平日 06:30–18:30 操作者上班不在，這段時間可以自己跑**（他 2026-09-21 給的）。
他說「回來了」或拔 USB 線就停。

### 省額度的三條規則（操作者擔心額度，PM 2026-09-21 轉達）

1. **整天最多聯絡 PM 一次**——收工時發一則合併回報（逐輪原始計數表、判定結果、
   各階段過沒過、路徑）。**做法與判定表昨晚都裁決完了，不要中途問。**
   卡死而且不是自己能決定的，**去找操作者，不要找 PM**。
2. **跑劇本時不要盯畫面**：runner 放背景，跑完**只讀文字報告**（含 `satisfied_by` 的 WARN）。
   **某一步失敗了才開那一步的截圖。不要輪詢、不要逐步看圖。**
3. **大量讀取交給 Sonnet 子 agent**（讀 log 全文、disasm、畫面矩陣標註），主力只讀摘要。

### 優先序

1. **投射物實驗的第 0、1、2 輪**（計畫已核准：
   `research/2026-09-21-netspeed-host-patch/round2-plan.md`，**可直接開跑**）
   - 先做修補工具（驗雜湊→備份→逐 byte 比對→改→輸出新雜湊→`--restore`），**只對副本**
   - **第 0 輪**：只換 `Engine.dll`，看能不能開到登入畫面（測完整性檢查 ⬜）
   - **第 1 輪**：C 段完整跑，房主 log 應變成 `Client netspeed is 100000`
     （順便驗證昨天修的 `leave_battle` 第二層）
   - **第 2 輪**：A/B/A/B/A/B 各 3 輪，加入者開火、主武器 `MTE_a`（砲）、地圖 `Map_PC04`
2. **按鍵巨集探測**（約 20 分鐘，backlog 有契約概念：`User.ini` 綁一鍵串接主控台指令）
3. **`BRIDGE-SPIKE`** 排最後。**操作者如果說額度緊，直接順延，不用問 PM。**

### 套用的修補：候選 1b（已核准，把關已通過）

`Engine.dll`（**只改副本**），三處，共 10 bytes：

| VA | file offset | 原始 | 新 |
|---|---|---|---|
| `0x1047f9b1` | `0x17f9b1` | `08 07 00 00` | `A0 86 01 00` |
| `0x1047f9b5` | `0x17f9b5` | `7d 07` | `90 90` |
| `0x1047f9b8` | `0x17f9b8` | `08 07 00 00` | `A0 86 01 00` |

stock sha256 `fc51fe1240ee34…`。三處現值昨晚已 hex-level 核對過。
設計與把關依據：`research/2026-09-21-netspeed-host-patch/design.md`。
