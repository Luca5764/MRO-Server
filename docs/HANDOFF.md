...
## ▶️ 現況快照（2026-09-22 12:45 收工）

**主線的投射物問題已經解決並量化完畢。** 接手第一件事是 `docs/backlog.md` 的 **NETSPEED-MIN**。

### 今天做完的事（全部已 commit 在 `reverse-work`）

| | 結論 |
|---|---|
| 第 0 輪 | ✅ 客戶端對 `Engine.dll` **沒有**啟動期完整性檢查 |
| 第 1 輪 | ✅ 候選 1b 讓房主記的 netspeed 10000 → 100000（逐 byte 覆核過 log 印的就是寫進 `+0x50` 的值） |
| **第 2 輪** | ✅ **投射物遺失歸零**：自動化 A 16/71＝22.5% vs B 0/49＝0%；人手 A′ 10/34＝29.4% vs B′ 0/34＝0% |

附帶確認：`MTE_a` `FireRate=1.5s`／`AmmoNumInitial=68`（一條命 34 次扣扳機）、
`HitLoc=== >` 是**每次扣扳機一行**、**單人基準 0%**、Pico 每次 `CLICK` 固定開銷 **3.44 秒**、
F 鍵**不會**選機體、`select_map` 多餘（建房預設就是 `Map_PC04` 初級）、
補彈是 `SPMaxUP_BD`（SP→9999）＋F3（50 SP）。

詳見 `docs/journal/2026-09-22-*.md`（六篇），逐輪台帳在
`docs/research/2026-09-21-netspeed-host-patch/round2-run-tally.txt`。
...
- 測試帳號 5、6 的 `items` 中 **`mech_type 1` 主武器已換成 `MTE_a`（`25300101`）**
  還原：`node "Metal Rage Online Server/database/swap-test-mech1-weapon.js" --revert --apply`
- `mech_licenses` 是**原始順序**（當初的槽位對調已還原，該腳本無效但保留備用）
- **主安裝完全沒有被碰過**（sha256 仍是原廠 `fc51fe12…`）

### 下一步（PM 2026-09-22 指定）

**NETSPEED-MIN**（契約在 `docs/backlog.md` 最後）：找夠用的最小值。先試 30000、3 輪，夠就停。
`patch_netspeed_host.py` 要加 `--value` 參數，**不要手改 bytes**。
**不急，額度緊可以順延，不必問 PM。**

> 為什麼重要：100000 是**每條連線**的上限，8 人房房主上行最壞約 800 KB/s，
> 走 Hamachi 中繼可能撐不住。**在找出最小值之前，不要把 100000 的版本發給朋友。**

### 還沒證明的事（不要當成已解決）

- ⬜ 100000 是否**必要**（只證明 10000 不夠、100000 夠）
- ⬜ 只測過**同機迴路**、只測過 **2 人**
- ⬜ 修補過的 DLL 只驗過登入與一場 PvE；PvP、其他地圖、長時間運行未測
- 🟡 **雙成因假說有疑點**：同一修補同時消除孤立單發與成串失敗。
  H-AMMO-DESYNC 改列**待查、未標 ❌**（PM 2026-09-22 同意，不另開任務）

### 今天犯過的錯（照抄可以少踩一次）

1. **跑實驗時改了它依賴的 DB** —— 違反「一次只改一個變數」。等它跑完只差十分鐘。
2. **事後另拍截圖去推斷失敗原因** —— 拍到的是幾分鐘後的畫面，推出錯的成因。
   **一律用 runner 在失敗當下自存的那張。**
3. 拿 `MTE_a` 的 1.5 秒冷卻去解釋 `MOM_a`（實際 0.09 秒）的結果 —— 引錯武器的數值。
4. 客戶端關不掉時先想到「當掉了」，實際上是**還在戰鬥中、滑鼠被遊戲抓著**；
   先 ESC ＋兩層確認離開戰鬥就關得掉。
