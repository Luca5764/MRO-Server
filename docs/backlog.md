...
>
> ### ⚠️ 2026-09-22 再修訂（操作者決定，PM 轉達）——**升為最高優先，階段 2／3 改為實作**
>
> **A/B 與 30000+budget 收完就全力做這個，其他工具類任務全部排後。**
>
> 1. **階段 2、3 從「只出設計」改為實作**：
>    - 階段 2 交付：場景編號每秒一行的腳本 ＋ **`ServerFire`／`ClientFire` 計數腳本**。
>    - 階段 3 交付：**「從 JS 呼叫主控台指令」可用的腳本**，用 `netspeed`／`stat net` 驗證。
> 2. **按鍵巨集探測取消**，被階段 3 取代。
> 3. **1 天限時只套用在階段 1**；階段 1 過了，階段 2 和 3 **不設限時，做到能用**。
> 4. 今天 18:30 前做不完階段 1 就**明天 06:30 接著**。
> 5. **anti-attach 不繞、不碰 `MetalRage.exe`**；若被砍，記錄結束碼與 log 尾段後**停下來回報**。
...
## CLIENT-KIT-PATCH — 把 30000＋budget 放進發給朋友的包（PM 2026-09-22 定案後開立）

### 目標

讓朋友拿到的 client-kit 帶著已驗證的修補，而且**收到的人自己驗得出檔案沒被改過**。

### 範圍

1. 用 `tools/patch_netspeed_host.py --value 30000 --budget` 產出 `Engine.dll`，
   把**產出檔的 sha256** 與**原廠 sha256**（`fc51fe1240ee3411…`）都寫進 client-kit 的 README。
2. `setup-client.ps1` 加一步：**套用前驗原廠雜湊、套用後驗結果雜湊**，對不上就停。
3. README 要寫清楚：改了什麼（兩處 netspeed 立即數＋三處 budget）、為什麼、怎麼還原
   （`--restore`，或還原備份檔）。
4. **房主與加入者都要套同一份**（PM 裁決）。

### 背景

- 效果與證據：`journal/2026-09-22-1800-budget-ab.md`、`journal/2026-09-22-1345-netspeed-min.md`、
  台帳 `research/2026-09-22-netspeed-budget/threshold-tally.txt`。
- 人手連按四條件對照：原廠 10000 ＝ 29.4%、30000 單獨 ＝ 5.7%、100000 ＝ 0%、
  **30000＋budget ＝ 0%（34/34）**。
- 選 30000 不選 100000 的理由：100000 是**每條連線**上限，8 人房房主上行最壞約 800 KB/s，
  走中繼可能撐不住。

### 限制

- **不要在這個任務裡動主安裝**；要不要套由操作者決定（PM 裁決）。
- 雜湊一律實際算出來寫，不要抄這份契約裡的字串。
- 修補工具**不要**為了這件事改行為，只是拿來產檔。

### 交付

- 更新後的 client-kit README 與 `setup-client.ps1`
- 一篇日誌，附產出檔雜湊與驗證輸出

### 完成條件

1. 在乾淨的原廠 `Engine.dll` 上跑一次，產出檔雜湊與 README 寫的一致。
2. `setup-client.ps1` 對「原廠雜湊不符」與「套用後雜湊不符」兩種情況都會停。
3. ⬜ **加入者端套 budget 我們自己沒驗過**——A/B 六輪只套房主。
   PM 依 Moon 的做法裁決兩端都套，這一點要在 README 與日誌寫明是**採信外部來源**，
   不是我們量過的。

---

## NETSPEED-LOW-BUDGET（低優先，PM 2026-09-22 指定記下但現在不做）
...
## FPS-PRESENTMON — 用外部工具量 FPS，取代截圖讀數字（操作者 2026-09-22 同意裝工具）

### 目標
讓 FPS 變成**自動記錄的數字**，而不是人去看截圖上的 overlay。

### 為什麼
今天為了量掛鉤對 FPS 的影響，只能用 `stat fps` 的畫面 overlay ＋ 截圖判讀：
- 只有**單一瞬間**的值，看不到 frame-time 分布與卡頓（操作者目視回報「會掉到 75」，
  但我們沒有數據）；
- 每量一次就要人讀一張圖；
- **今天已經因為分母拿錯（待機值當實戰值、加入者當房主）連續更正兩次**
  （`journal/2026-09-22-1620-host-fps.md` 兩段更正）——自動記錄會讓「哪一台、什麼狀態」
  跟數字綁在一起，這類錯誤就不會再犯。

### 範圍
1. 裝 **PresentMon**（Intel，從外部量 D3D 程式的 frame time，**不用注入、不用截圖**）。
2. 包一層 `tools/win/fps.sh`（或併進既有的 `tools/win/`），能對指定行程名開始／停止取樣，
   輸出 CSV 或 JSONL。
3. 接進 Pico runner：一個動作開始取樣、一個動作停止並把統計（平均／p1／p99／樣本數）
   寫進 runner 報告。
4. 文件寫明：裝在哪、怎麼移除、量到的是哪個行程。
...
## DB-FAIL-PERREQ — DB 在運行中掛掉時，不要對客戶端偽裝登入成功

**目標**：啟動期已經會大聲失敗了（2026-09-23 已合併）。但 `dispatch/account.dispatch.js`
的 `handleLogin()` 還有**每請求**層級的靜默退路：`db.getAccountByUsername()` 若因為 DB 掛掉
而 reject，會被 `:372` 的 catch 吃掉，印一行 error 後走「DB unavailable - sending hardcoded
defaults」分支（`:374` 起），**對客戶端偽裝登入成功並送空帳號資料**。另外 `:252` 的
`catch (dbErr)` 讓 `createAccount()` 失敗時 `account` 保持 `null`，掉進同一套分支
（`:258-363` 與 `:373` 之後幾乎是複製貼上的兩份）。

**為什麼現在才做**：啟動期檢查保證了「開機時 DB 是通的」，所以這條路徑現在代表
**DB 在運行中掛掉**——是異常，不是常態。但玩家看到的會是「登入成功、東西全不見」，
跟 2026-09-22 那次一模一樣的誤導。

**要決定的（這是設計決定，要高階或 PM 拍板，不要讓 worker 自己選）**：
- (a) 照樣回應但**明顯記錄**：加一個獨特的 log 標記與計數，行為不變。最安全，但玩家端仍被誤導。
- (b) **不要偽裝成功**：回登入失敗，讓客戶端顯示錯誤。要先查清楚客戶端收到登入失敗會怎麼顯示
  （`docs/reference/client-ui.md`），而且會改變客戶端可見行為，**要實測**。
- (c) 直接斷線。最誠實，但對正在遊戲中的其他人影響未知。

**先做的**：不管選哪個，那兩份幾乎相同的空帳號分支（`:258-363` 與 `:374` 之後）應該先合併成
一份，否則之後改行為要改兩個地方、一定會漏掉一個。這一步可以先交 worker，是純重構，
