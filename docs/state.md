...
| 房主端 `Engine.dll` 修補（候選 1b）可把房主為每條連線記下的 `CurrentNetSpeed` 從 10000 強制成 **100000**：`0x1047f9b1` `cmp eax,0x708`→`cmp eax,100000`、`0x1047f9b5` `jge` NOP 掉、`0x1047f9b8` `mov eax,0x708`→`mov eax,100000`（file offset `0x17f9b1`/`0x17f9b5`/`0x17f9b8`；stock sha256 `fc51fe12…`→修補後 `a007507d…`）。log 行 `Client netspeed is %i` 印的就是寫進 `+0x50` 的同一個 `eax`（`push eax` 在 `0x1047f9c7`，`mov [edx+0x50],eax` 在 `0x1047f9c8`，`Logf` 在 `0x1047f9d8`）。**客戶端對 `Engine.dll` 沒有啟動期完整性檢查**（修補後正常開到登入畫面，log 無 checksum／tamper 訊息）。✅ **第 2 輪已定案：投射物遺失從 24.8% 歸零**——**自動化**（Pico，固定 3.45 秒間隔）A 16/71＝22.5% vs B 0/49＝0%；**人手**（連按到底，節奏不同，是另一個條件）A′ 10/34＝29.4% vs B′ 0/34＝0%；**合計** A 26/105＝24.8% [CI 17.5–33.8] vs B 0/83＝0.0% [CI 0–4.4]，逐輪不重疊（B 每輪 0% < A 最低輪 14.3%），Fisher p≈1.0e-7。**連成串失敗也一併消失**（人手連按到底：原廠 `....XXX.XX...X......X...XX......X.` 串 3,2,1,1,2,1；修補 34/34 無失敗）→ 🟡 雙成因假說有疑點，H-AMMO-DESYNC 改列待查、**先不要標 ❌**。單人基準已測為 0%，故雙人缺口可全數歸給送出閘門。❌ **「30000 單獨讓遺失歸零」已被推翻**（Sol 2026-09-22 跨公司審查）：自動化（Pico 固定間隔）三輪 36/36＝0% 成立，但同操作者同晚的**人手連按**輪只有 33/35（缺 2 發＝5.7%；`research/2026-09-22-netspeed-budget/manual-30000-run-190747.55.log` 重算 HitLoc 35、`MTE_a Fire` 33）。同條件加上 budget patch 後人手 34/34（`manual-30000-budget-run-192214.53.log`）。**因此發包值是 30000＋budget**，六處修改後 sha256 `f4b253a3…`（Sol 以原廠 DLL 重套後吻合 `tools/client-kit/setup-client.ps1:61-65`）；🟡 人手比較兩邊各只有一輪，結論範圍限**兩人同機 PvE**。⬜ 30000 是「夠用」不是「最小」，15000 以下未測；⬜ 只測過同機迴路與 2 人。修補只套在副本 `C:\Games\MetalRage Online 2`，主安裝未動 | ✅ [DLL] `0x1047f9b1`–`0x1047f9d8`（逐 byte 覆核原廠備份）＋ [LOG] 同一劇本前後對照：原廠 `run-210802.96.log`/`run-221608.34.log` 皆 `Client netspeed is 10000` ×2，修補後 `run-65417.98.log` `Client netspeed is 100000` ×2 ＋ [SHOT] `shots/round0-login.png`（Sol 2026-09-22 跨公司審查：候選 1b 的 bytes、控制流、host/joiner 分流與 100000 A/B 成立；30000 單獨歸零 ❌） | `journal/2026-09-22-0640-engine-dll-patch-round0.md`、`journal/2026-09-22-0703-round1-host-netspeed-100000.md`、`journal/2026-09-22-1345-netspeed-min.md` |
| 房主端 `Engine.dll` **budget patch**（Moon 提供、我方核對）：`UNetConnection::Tick`（匯出表 `0x1042dda0 = ?Tick@UNetConnection@@UAEXXZ`）裡 `QueuedBytes` 的下限從 `-2D` 放寬成 `-(2D + CurrentNetSpeed/4)`（D＝DeltaTime×CurrentNetSpeed）。做法是 `0x1042e1f8` 跳到 `.text` 尾端的 cave `0x10378b20`，重現被蓋掉的 `fadd st(0),st(0)` 與 `mov ecx,[esi+0x14c]`，中間用 x87 `fild/faddp` 加上 `CurrentNetSpeed>>2`，再跳回 `0x1042e200`；`[ebp-0x1c]` 的借用沒有 live range 重疊（cave 讀完才返回，原碼在 `0x1042e204` 才覆寫）。工具 `tools/patch_netspeed_host.py --budget`（同時把 `.text` VirtualSize `0x377b1e`→`0x378000`）。NetSpeed 30000 時放寬量固定為 7500 bytes | **靜態機制 ✅ [DLL]** `0x1042e1ee`–`0x1042e207`、cave `0x10378b20`（Sol 2026-09-22 跨公司審查：逐指令成立，函式身分定案為 `UNetConnection::Tick`）；**實驗歸因 🟡 [TEST]** 15000 交錯 A/B：A1–A3 4/37 vs B1–B3 0/36（Fisher two-sided p≈0.115），加入 A4 後 A 8/53 vs B 0/36（p≈0.0192）——只有 3 個 B 輪、A4 是看過結果才追加，**不足以標成「budget 一般性消除遺失」**。「code cave 不會讓客戶端崩」只有 15000＋budget 三場、30000＋budget 各一場自動化／人手支持，長時間、跨網、多人未測 | `research/2026-09-22-netspeed-budget/high-tier-review.md`、`threshold-tally.txt`、`journal/2026-09-22-1800-budget-ab.md` |
...
| 項目 | 狀態 | 依據 |
|---|---|---|
| `Death_SN 0x00230124` 廣播給加入者**安全**。兩道保護：呼叫 GameInfo 事件那段被 `Game_Host_Check()`（`0x1071a560`：`return *(this+0xfac) & 1`）包住，加入者旗標 false 整段不執行（`0x1072e208`）；另一段由 `GIsClient` 把關、對加入者會執行的是 `eventTreatKillMSG_UJ`（`0x1072e1d6`），逐層指標都有 null 檢查、遇 null 就跳過 | ✅ [DLL]（`0x107db912`→`Game_Action_Death` 真身 `0x1072e120`）[LOG] 同機雙開 9 份加入者 `run-*.log` grep `ScriptWarning`／`Accessed None` 零命中（Sol 2026-09-22 跨公司審查：成立） | `research/2026-09-21-moon-objective-protocol/verify-b-death-sn.md` |
| `EndRound_SN 0x00222211` 廣播給加入者**安全**，但機制**不同**：`Game_End_Round`（`0x1072e310`）沒有 host 閘門，呼叫 `[Level vtable]+0xb4` ＝ `ULevel::EndRound_BD`（`Engine.dll 0x1047ab70`）；該函式讀 `LevelInfo+0x630`（GameInfo）後有顯式 `test esi,esi / je` null 檢查，加入者 null 就整段跳過 | ✅ [DLL] 虛表 `??_7ULevel@@6BUObject@@@ 0x10691af8`；同表 `+0xa0` ＝ `ULevel::Listen 0x1047a5e0`，與 netspeed 調查獨立對上（Sol 2026-09-22 跨公司審查：成立） | `research/2026-09-21-moon-objective-protocol/verify-b2-endround-vtable.md` |
| `User_Score_SN 0x00222221` 對加入者**安全** | ✅ [DLL] 完整反組譯 `0x107ece60`–`0x107ed15f`，只寫 `UZNetwork_DJ` 自己的欄位／陣列，**全函式沒有 `0x108e550c`、沒有 Level、沒有物件 vtable 呼叫**（Sol 2026-09-22 跨公司審查：成立） | `verify-d-endgame.md` |
| `EndGame_SN 0x00222213` 對加入者**安全（我方實際走到的分支內）** | ✅ [DLL] `0x107d806a` 用 `GIsClient` 分兩支；**我方 host 與 joiner 都是 `GIsClient!=0`**，只走 `Game_End_Battle`→`Community_Chat_Clear`（純自身陣列）→`Event_Call`（純自身佇列，有 null 檢查）→`Scene_Change`（自身欄位＋有 null 檢查的監聽者廣播）。唯一會碰 `Level` vtable 的 `Dedi_End`（`0x10716e90`）開頭就是 `if (GIsClient!=0) return`，**我方永遠不可達**（Sol 2026-09-22 跨公司審查：成立） | 同上 |
