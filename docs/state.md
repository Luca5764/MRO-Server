...
| 房主端 `Engine.dll` 修補（候選 1b）可把房主為每條連線記下的 `CurrentNetSpeed` 從 10000 強制成 **100000**：`0x1047f9b1` `cmp eax,0x708`→`cmp eax,100000`、`0x1047f9b5` `jge` NOP 掉、`0x1047f9b8` `mov eax,0x708`→`mov eax,100000`（file offset `0x17f9b1`/`0x17f9b5`/`0x17f9b8`；stock sha256 `fc51fe12…`→修補後 `a007507d…`）。log 行 `Client netspeed is %i` 印的就是寫進 `+0x50` 的同一個 `eax`（`push eax` 在 `0x1047f9c7`，`mov [edx+0x50],eax` 在 `0x1047f9c8`，`Logf` 在 `0x1047f9d8`）。**客戶端對 `Engine.dll` 沒有啟動期完整性檢查**（修補後正常開到登入畫面，log 無 checksum／tamper 訊息）。⚠️ **對投射物遺失有沒有改善完全沒測**，那是第 2 輪的事；`netspeed` 只是中間量。修補只套在副本 `C:\Games\MetalRage Online 2`，主安裝未動 | ✅ [DLL] `0x1047f9b1`–`0x1047f9d8`（逐 byte 覆核原廠備份）＋ [LOG] 同一劇本前後對照：原廠 `run-210802.96.log`/`run-221608.34.log` 皆 `Client netspeed is 10000` ×2，修補後 `run-65417.98.log` `Client netspeed is 100000` ×2 ＋ [SHOT] `shots/round0-login.png`（**未經跨公司審查**） | `journal/2026-09-22-0640-engine-dll-patch-round0.md`、`journal/2026-09-22-0703-round1-host-netspeed-100000.md` |
