...
| 房主端 `Engine.dll` 修補（候選 1b）可把房主為每條連線記下的 `CurrentNetSpeed` 從 10000 強制成 **100000**：`0x1047f9b1` `cmp eax,0x708`→`cmp eax,100000`、`0x1047f9b5` `jge` NOP 掉、`0x1047f9b8` `mov eax,0x708`→`mov eax,100000`（file offset `0x17f9b1`/`0x17f9b5`/`0x17f9b8`；stock sha256 `fc51fe12…`→修補後 `a007507d…`）。log 行 `Client netspeed is %i` 印的就是寫進 `+0x50` 的同一個 `eax`（`push eax` 在 `0x1047f9c7`，`mov [edx+0x50],eax` 在 `0x1047f9c8`，`Logf` 在 `0x1047f9d8`）。**客戶端對 `Engine.dll` 沒有啟動期完整性檢查**（修補後正常開到登入畫面，log 無 checksum／tamper 訊息）。✅ **第 2 輪已定案：投射物遺失從 24.8% 歸零**——**自動化**（Pico，固定 3.45 秒間隔）A 16/71＝22.5% vs B 0/49＝0%；**人手**（連按到底，節奏不同，是另一個條件）A′ 10/34＝29.4% vs B′ 0/34＝0%；**合計** A 26/105＝24.8% [CI 17.5–33.8] vs B 0/83＝0.0% [CI 0–4.4]，逐輪不重疊（B 每輪 0% < A 最低輪 14.3%），Fisher p≈1.0e-7。**連成串失敗也一併消失**（人手連按到底：原廠 `....XXX.XX...X......X...XX......X.` 串 3,2,1,1,2,1；修補 34/34 無失敗）→ 🟡 雙成因假說有疑點，H-AMMO-DESYNC 改列待查、**先不要標 ❌**。單人基準已測為 0%，故雙人缺口可全數歸給送出閘門。✅ **100000 並非必要：30000 就夠**——同劇本同條件三輪 36/36、缺口 0.0%，對照原廠 16/71＝22.5%（逐輪最低 14.3%），房主 log 三輪都印 `Client netspeed is 30000`（值落地與缺口歸零兩條獨立證據），修補後 sha256 `dc2f824d…`；⬜ 30000 是「夠用」不是「最小」，15000 以下未測；⬜ **人手連按（成串失敗那個形態）在 30000 下未測**；⬜ 只測過同機迴路與 2 人。修補只套在副本 `C:\Games\MetalRage Online 2`，主安裝未動 | ✅ [DLL] `0x1047f9b1`–`0x1047f9d8`（逐 byte 覆核原廠備份）＋ [LOG] 同一劇本前後對照：原廠 `run-210802.96.log`/`run-221608.34.log` 皆 `Client netspeed is 10000` ×2，修補後 `run-65417.98.log` `Client netspeed is 100000` ×2 ＋ [SHOT] `shots/round0-login.png`（**未經跨公司審查**） | `journal/2026-09-22-0640-engine-dll-patch-round0.md`、`journal/2026-09-22-0703-round1-host-netspeed-100000.md`、`journal/2026-09-22-1345-netspeed-min.md` |
