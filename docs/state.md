...
| 項目 | 狀態 | 依據 |
|---|---|---|
| 30907 登入的身分：Gate `Leave_SA 0x00220132` body+0x06／+0x0A 兩個 u32，客戶端透過 `Certify_Away_Set`（`0x10715f70`）存起來，再放進 `Login_Again_CQ 0x00110124` body+0／+4 原樣帶回。目前伺服器送 0，所以用 last_login 猜帳號 | ✅ [DLL] `0x107dc7d3`–`0x107dc846`、`0x107c3ef5`；[LOG] body 全 0（未經跨公司審查；PM 機械核對相符） | `journal/2026-09-18-2350-game-login-token-chain.md` |
