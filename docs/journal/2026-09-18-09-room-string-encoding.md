...
- [OBS][SHOT] `shots/r5-state.png`：開啟 R2 後房間標題由 `[1]L` 恢復為 `[1]Lucas`。
- [OBS] 這次確認 `0x0022021A` 的房名輸入應走 ANSI／窄字串；R2 的共用 sender 路徑有效。
- [OBS] 這個結果推翻 `AGENTS.md` 現有「房間名稱是 UTF-16LE」的概括筆記（至少對 `0x0022021A` 不成立）。
- [限制] `AGENTS.md` 由高階負責修改，本階段只在本篇留下更正依據，未改規則檔。
- [TEST] Claude 高階已完成伺服器與截圖驗證；本篇由待審改為 ✅，開關固定為 enabled。
