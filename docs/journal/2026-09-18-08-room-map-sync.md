...
- [OBS][SHOT] `shots/r1-map.png`：開啟 R1 後，房間面板由寫死的 9001「動力奪取戰」改為實際選中的 9010「潛入作戰」。
- [OBS][SHOT] 同一畫面中任務簡報與清單選中項也跟隨 9010，符合房間面板使用實際 map id 的預期。
- [LOG] 實測記錄 `SN_ROOM_DEFAULT mapIndex=9010`、`SN_MAP_CHANGE_ALL selectedIdx=9`；清單第 9 項是 9010。
- [TEST] Claude 高階已操作伺服器與客戶端完成驗證；本篇由待審改為 ✅，開關固定為 enabled。
