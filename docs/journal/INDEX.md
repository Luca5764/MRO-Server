...
| 2026-09-19 | `2026-09-19-0100-d1-step1-rooms.md` | `0x00220201`、`0x00220234` | — | D1-1（中階，待審）：新增 `rooms.js` 房間登錄表，`CQ_CREATE` 雙寫 Room＋host member，Leave_CQ／socket close 分別 `removeMember`／`setMemberClient(null)`；房號生成移入 `rooms.allocateRoomId()`；回歸 4 樣本全綠＋新增 `test/rooms.js` 5 案例全過；`rooms._resetForTests()` 已接進 golden harness 的每樣本重置。 |
