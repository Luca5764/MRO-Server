...
> **2026-09-22 遮蔽**：本篇一處筆電真實 Radmin VPN 位址已換成 `<LAPTOP_VPN_IP>`。原值曾於 2026-09-20 進入 git 歷史並已推上 origin；依 PM 裁決不改寫已公開歷史（同 `2026-09-20-1530-m3r-vpn-rehearsal.md`）。
...
**第 3 項 — VPN 本身的掉包率。** [TEST] 桌機 → 筆電 Radmin IP `<LAPTOP_VPN_IP>`，`ping -n 200`：**傳送 200、收到 200、遺失 0（0%）**，來回時間 20／43／189 ms（最小／平均／最大）。→ **不是線路掉包。** 但抖動將近 10 倍，與筆電走手機熱點一致。
...
## 更正（19:05）：`sup1` 那一段切錯了，H-AMMO-DESYNC 的 ❌ 收回

[OBS] 操作者事後指出：**那台機體的主武器也是一次扣兩發彈藥**（與角色對調那輪的 `MTE_a` 相同機制，只是不同武器）。上面「追加（18:40）」把 96 發彈藥當成 96 次扣扳機，是錯的——**96 發彈藥＝48 次扣扳機**。`HitLoc===` 是一次扣扳機一行，所以那一段 138 行分別是基準 48、`sup1` 之後 90。

重切同一批資料：

| 切法 | 基準 | `sup1` 之後 |
|---|---|---|
| 舊（誤） | 96 扣／缺 13 → 13.5% | 42 扣／缺 5 → 11.9% |
| **正確** | **48 扣／缺 10 → 20.8%**，失敗串 `1,3,2,2,1,1` | **90 扣／缺 8 → 8.9%**，失敗串 `1×8` |

Fisher 精確檢定 p ≈ 0.063（未達顯著，但方向明確）。

除了比率，**失敗的形狀也變了**：基準段有長度 3、2、2 的連續失敗，`sup1` 段**全部是孤立單發**。

→ **收回 ❌ H-AMMO-DESYNC，改回 🟡。** 目前最能自洽的是雙成因：
1. **成串的失敗** ← `HasAmmo()` 閘門（彈藥不同步），`sup1` 消掉了；
2. **殘留的孤立單發（約 9%）** ← 回程 `ToAll`。
