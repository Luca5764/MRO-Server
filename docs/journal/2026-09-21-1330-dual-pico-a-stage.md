...
## 追加（2026-09-21）：上面那條規則已經取消

本篇「規則落差」那一節記的是我自己補進政策的「同一段改過程式再跑算新的一輪、
要重新確認在場」——**操作者已於 2026-09-21 取消整個「在場」規則**
（原話：「本地而且根本不會影響到其他人的專案」），見
`reference/unattended-policy.md`。

**這筆違規紀錄仍然保留**：當時的規則是那樣寫的，我沒有照做，這件事本身照實記著。
規則之後被取消，不改變當時的事實。
