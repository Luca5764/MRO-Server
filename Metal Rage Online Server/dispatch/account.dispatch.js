        ...
        try {
            ...
            if (!account) {
                ...
                try {
                    ...
                    createAccountErr = dbErr;
                    console.error(`[ZDispatchAccount::CQ_LOGIN_WASABII] DB auto-create failed:`, dbErr.message);
                    // Fall through - account will still be null, handled by the disconnect below.
                }
            }

            if (!account) {
                // DB-FAIL-PERREQ (docs/backlog.md, PM decision 2026-09-23): this
                // used to send hardcoded empty account data and let the client
                // think login succeeded ("登入成功、東西全不見" bug, wasted
                // half a day on 2026-09-22). Startup now verifies the DB is up
                // (database/db.js verifyConnectionOrExit()), so reaching here
                // means the DB broke mid-run -- treat it as the error it is:
                // disconnect this request's own connection (9211 login is
                // per-connection, doesn't affect anyone else already in a
                // match) and log loudly instead of faking success.
                const step = createAccountErr ? 'db.createAccount' : 'db.getAccountByUsername (post-create, no throw)';
                const code = createAccountErr && createAccountErr.code;
                console.error(`[ZDispatchAccount::CQ_LOGIN_WASABII] ERROR: DB failure for "${username}" at step=${step} code=${code} - disconnecting instead of faking login success`);
                client.disconnect();
                return;
            }
            ...
            // DB-FAIL-PERREQ (docs/backlog.md, PM decision 2026-09-23): used
            // to fake a successful login with hardcoded empty defaults here.
            // Now disconnects this request's connection and logs loudly
            // instead. KNOWN LIMITATION: the SA_LOGIN_WASABII success ack at
            // :226-231 above already went out before this DB call failed
            // (whitelist check passes -> ack sent -> THEN
            // db.getAccountByUsername() or another awaited DB call rejects),
            // so the client sees "success ack, then immediate disconnect"
            // rather than a clean login-failure response. Moving where
            // :226-231 sends would touch the normal login path -- out of
            // scope here; see docs/backlog.md DB-FAIL-PERREQ option (b) for
            // the follow-up once client display of a real login-failure
            // packet is confirmed.
            const username = client.username_ || 'Player';
            console.error(`[ZDispatchAccount::CQ_LOGIN_WASABII] ERROR: DB failure for "${username}" at step=login (post-ack) code=${err.code} - disconnecting instead of faking login success:`, err.message);
            client.disconnect();
        }
    }
