...
| header 是 BE，body 幾乎都是 LE；字串編碼**一個封包一個樣，一律照 DLL 確認**。已知：`Room_Name_SN 0x0022021A` 是 **ANSI**（客戶端自己呼叫 `winToUNICODE` 轉；送 UTF-16LE 會只顯示第一個字，2026-09-18 實測更正），`User_Default_SN`／`Game_User_SN` 的暱稱是 ASCII，`User_Name_SN 0x00220421` 名稱（body+0x1B）也是 **ANSI**（`0x107eb0ce` 呼叫 `winToUNICODE`，2026-09-18 實測） | `reference/protocol.md`；`journal/2026-09-18-*-room-string-encoding` |
